...
device = "cpu"
filepath = "AdditionalFiles/RemadeGraphs/"
...
classifier.load_state_dict(torch.load("Code/Transformer/FullEncoderOutputs/" + 'best_model.pth'))
classifier.eval()

print("2")

#Evaluation of 3 day mixed
ThreeDayMixedCloudCover, FullEncoderThreeDayMixedPredictions, MixedTime = TestingLoop(classifier, epochsRan, testloader=ThreeDayMixed_loader, filepath=filepath + "DayGraphs/" + "Mixed/", device=device, dataSplit=xThreeDayMixed, scaler_x=scaler_x) 

print("3")

#Evaluation of 3 day full
ThreeDayFullCloudCover, FullEncoderThreeDayFullPredictions, FullTime = TestingLoop(classifier, epochsRan, testloader=ThreeDayFull_loader, filepath=filepath + "DayGraphs/" + "Full/", device=device, dataSplit=xThreeDayFull, scaler_x=scaler_x) 
print("4")
#Evaluation of 2 day sunny
TwoDaySunnyCloudCover, FullEncoderTwoDaySunnyPredictions, SunnyTime = TestingLoop(classifier, epochsRan, testloader=TwoDaySunny_loader, filepath=filepath + "DayGraphs/" + "Sunny/", device=device, dataSplit=xTwoDaySunny, scaler_x=scaler_x) 
...
classifier.load_state_dict(torch.load("Code/Transformer/SimpleTransformerOutputs/" + 'best_model.pth'))
classifier.eval()

print("2")

#Evaluation of 3 day mixed
ThreeDayMixedCloudCover, SimpEncoderThreeDayMixedPredictions, MixedTime = TestingLoop(classifier, epochsRan, testloader=ThreeDayMixed_loader, filepath=filepath + "DayGraphs/" + "Mixed/", device=device, dataSplit=xThreeDayMixed, scaler_x=scaler_x) 

print("3")

#Evaluation of 3 day full
ThreeDayFullCloudCover, SimpEncoderThreeDayFullPredictions, FullTime = TestingLoop(classifier, epochsRan, testloader=ThreeDayFull_loader, filepath=filepath + "DayGraphs/" + "Full/", device=device, dataSplit=xThreeDayFull, scaler_x=scaler_x) 
print("4")
#Evaluation of 2 day sunny
TwoDaySunnyCloudCover, SimpEncoderTwoDaySunnyPredictions, SunnyTime = TestingLoop(classifier, epochsRan, testloader=TwoDaySunny_loader, filepath=filepath + "DayGraphs/" + "Sunny/", device=device, dataSplit=xTwoDaySunny, scaler_x=scaler_x) 
...
ax2.plot(CNNLSTMtest_predictions, label='CNNLSTM Predicted', color='red', ls="--")
ax2.plot(LSTMtest_predictions, label='LSTM Predicted', color='cyan', ls="--")
ax2.plot(CNNtest_predictions, label='CNN Predicted', color='green', ls="--")
ax2.set_ylabel("PV (kWh)", color='blue') 
ax2.tick_params(axis='y', labelcolor='blue')
...
classifier = newForecastingModel().to(device)


classifier.load_state_dict(torch.load("Code/Transformer/FullEncoderOutputs/" + "Pretrained/"+ 'best_model.pth'))
classifier.eval()

_, FullEncoderTwoDaySunnyPredictions, SunnyTime = TestingLoop(classifier, epochsRan=epochs, testloader=TwoDaySunny_loader, filepath=filepath + "Pretrained/RealDayGraphs/" + "Sunny/", device=device, dataSplit=xTwoDaySunny, scaler_x=scaler_x) 
...
classifier.load_state_dict(torch.load("Code/Transformer/SimpleTransformerOutputs/" + "Pretrained/"+ 'best_model.pth'))
classifier.eval()

_, SimpleTwoDaySunnyPredictions, SunnyTime = TestingLoop(classifier, epochsRan=epochs, testloader=TwoDaySunny_loader, filepath=filepath + "Pretrained/RealDayGraphs/" + "Sunny/", device=device, dataSplit=xTwoDaySunny, scaler_x=scaler_x) 
# Evalutaion(TwoDaySunnyPredictions, cloud_cover[5:55], scaler_y, yTwoDaySunny, history, filepath=filepath + "Pretrained/RealDayGraphs/" + "Sunny/", title="2 Sunny Days")
...
ax1.bar(range(len(cloud_cover[5:55])), cloud_cover[5:55], label='Cloud Cover', alpha=0.3, width=1.0,zorder=3)
ax1.set_ylabel("Cloud Cover (%)")

ax2.plot(scaler_y.inverse_transform(ytest.reshape(-1, 1)).flatten(), label='Actual', color='blue', lw=4, zorder=2)
ax2.plot(CNNLSTMtest_predictions, label='CNNLSTM Predicted', color='red', ls="--",zorder=2)
ax2.plot(LSTMtest_predictions, label='LSTM Predicted', color='cyan', ls="--",zorder=2)
ax2.plot(CNNtest_predictions, label='CNN Predicted', color='green', ls="--",zorder=2)
ax2.set_ylabel("PV (kWh)", color='blue') 
ax2.tick_params(axis='y', labelcolor='blue')
...
ax1.bar(range(len(cloud_cover[230:300])), cloud_cover[230:300], label='Cloud Cover', alpha=0.5, width=1.0, zorder=1)
ax1.set_ylabel("Cloud Cover (%)")


ax2.plot(scaler_y.inverse_transform(ytest.reshape(-1, 1)).flatten(), label='Actual', color='blue', lw=4, zorder=3)
ax2.plot(CNNtest_predictions, label='CNN Predicted', color='green', ls="--", zorder=4)
ax2.plot(CNNLSTMtest_predictions, label='CNNLSTM Predicted', color='red', ls="--", zorder=4)
ax2.plot(LSTMtest_predictions, label='LSTM Predicted', color='orange', ls="--",zorder=4)
ax2.set_ylabel("PV (kWh)", color='blue') 
ax2.tick_params(axis='y', labelcolor='blue')
