...
model1 = Sequential()
model1.add(LSTM(64, activation='tanh', return_sequences=True, input_shape=(5,15)))
model1.add(Dropout(0.2))
model1.add(LSTM(32, activation='tanh', return_sequences=True))
model1.add(Dropout(0.2))
model1.add(LSTM(16, activation='tanh', return_sequences=False))
model1.add(Dense(16, 'relu'))
model1.add(Dense(1, 'linear'))

model1.compile(loss=MeanSquaredError(), optimizer=Adam(learning_rate=0.0001), metrics=[RootMeanSquaredError(), 'mean_absolute_percentage_error'])
