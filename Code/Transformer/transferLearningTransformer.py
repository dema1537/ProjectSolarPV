...
for epoch in range(epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        epochs_no_improve = 0
        torch.save(newModel.state_dict(), filepath + 'newTLbest_model.pth')
    else:
        epochs_no_improve += 1
        if epochs_no_improve >= patience:
            print(f"Early stopping triggered after {epoch+1} epochs!")
            newModel.load_state_dict(torch.load(filepath + 'newTLbest_model.pth'))
            break
...
with torch.no_grad():
    for batch in test_loader:
        ...
        x_batch, y_batch = batch
        x_batch, y_batch = x_batch.to(device), y_batch.to(device)
        test_predictions = newModel(x_batch).flatten()
...
with open(filepath + "newTLForecastingTestMetrics.txt", "w") as f:
...
lossFunction = nn.MSELoss()
optimiser = optim.Adam(oldrunModel.parameters(), lr=1e-4, weight_decay=1e-3)
scheduler = ReduceLROnPlateau(optimiser, 'min', factor=0.5, patience=3, verbose=True)
...
for epoch in range(epochs):
    ...
    for batch in train_loader:
        ...
        optimiser.zero_grad()  
        predictions = oldrunModel(x_batch).flatten()  
    ...
    with torch.no_grad():
        for batch in val_loader:
            x_batch, y_batch = batch
            x_batch, y_batch = x_batch.to(device), y_batch.to(device)
            val_predictions = oldrunModel(x_batch).flatten()
            loss = lossFunction(val_predictions, y_batch.flatten())
            val_loss += loss.item()
    ...
    if avg_val_loss < best_val_loss:
        best_val_loss = avg_val_loss
        epochs_no_improve = 0
        torch.save(oldrunModel.state_dict(), filepath + 'oldTLbest_model.pth')
    else:
        epochs_no_improve += 1
        if epochs_no_improve >= patience:
            print(f"Early stopping triggered after {epoch+1} epochs!")
            oldrunModel.load_state_dict(torch.load(filepath + 'oldTLbest_model.pth'))
            break
...
with torch.no_grad():
    for batch in test_loader:
        x_batch, y_batch = batch
        x_batch = x_batch.to(device)
        outputs = oldrunModel(x_batch)
        predictions.extend(outputs.squeeze().tolist())

        x_batch, y_batch = batch
        x_batch, y_batch = x_batch.to(device), y_batch.to(device)
        test_predictions = oldrunModel(x_batch).flatten()
        loss = lossFunction(test_predictions, y_batch.flatten())
        test_loss += loss.item()
...
with open(filepath + "oldTL_TransformerTestMetrics.txt", "w") as f:
    f.write(f"Test Loss: {avg_test_loss:.6f}\n")
    f.write(f"Test RMSE: {avg_test_rmse:.4f}\n")
    f.write(f"Test MAPE: {avg_test_mape:.2f}%\n")
    f.write(f"Test epoch: {epochs:.2f}%\n")
